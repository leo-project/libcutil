...
static void normal_test(lcu_cache_opt opt, size_t limit, double factor, size_t min, size_t max) {
    lcu_cache* lc = lcu_cache_init(opt, limit, factor, min, max);
    byte keystr[16];
    byte valstr[16];
    String key = {keystr, 3};
    String val = {valstr, 0};
...
START_TEST(put_already_there) {
    lcu_cache* lc = lcu_cache_init(auto_eject_on, 1024 * 1024 * 64, 2, 128, 1024 * 1024);
...
START_TEST(put_beyond_limit) {
    lcu_cache* lc = lcu_cache_init(auto_eject_on, 64, 2, 16, 64);
...
START_TEST(bulkload) {
    lcu_cache* lc = lcu_cache_init(auto_eject_on, 1024 * 1024 * 64, 1.5, 128, 1024 * 1024);
